export const STORE = {
  name: "Italino",
  legalName: "ФОП Оберемчук Сергій Олександрович",
  /** Для речення «сайт належить …» в оферті й умовах — його вимагає RozetkaPay. */
  legalNameDative: "фізичній особі-підприємцю Оберемчуку Сергію Олександровичу",
  /** Адреса сайту в юридичних текстах — фіксована, не з env, щоб не залежати від оточення збірки. */
  siteUrl: "https://italino.shop",
  taxId: "3121116950",
  edrRecord: "2011600000000066020",
  edrDate: "03.07.2026",
  address:
    "Україна, 35610, Рівненська область, Дубенський район, село Сатиїв, вул. Миру, будинок 2",
  phone: "+380 97 044 72 29",
  phoneHref: "tel:+380970447229",
  email: "serhiioberemchuk@gmail.com",
  hours: "щодня з 09:00 до 21:00",
  paymentProvider: "RozetkaPay",
} as const;

export const ROZETKAPAY_PAYMENT_KEY = "rozetkapay";

