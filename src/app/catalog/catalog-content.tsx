...
export async function CatalogContent({ searchParams, category }: { searchParams: Promise<Record<string, string | string[] | undefined>>; category?: string }) {
  ...
  const filtered = all.filter((p) => {
    const haystack = `${p.name} ${p.sku ?? ""} ${p.category?.name ?? ""}`.toLocaleLowerCase("uk");
    return (!query || haystack.includes(query)) && (!brand || p.brand?.name === brand) && (!terms.length || terms.some((term) => haystack.includes(term)));
  });
  ...
  return (
    <div className={styles.catalogLayout}>
      <form className={styles.filters}>
        ...
        <label>Бренд<select className={styles.select} name="brand" defaultValue={brand}><option value="">Усі бренди</option>{brands.map((name) => <option key={name}>{name}</option>)}</select></label>
        <label>Сортування<select className={styles.select} name="sort" defaultValue={sort}><option value="newest">Новинки</option><option value="price_asc">Ціна: від нижчої</option><option value="price_desc">Ціна: від вищої</option></select></label>
