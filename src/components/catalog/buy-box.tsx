...
export function BuyBox({ variants }: { variants: CrmProduct[] }) {
  ...
  return <>
    ...
    {variants.length > 1 ? <fieldset className={styles.optionGroup}><legend>Оберіть варіант</legend><div className={styles.chips}>{variants.map((variant) => <label className={styles.chip} key={variant.id}><input type="radio" name="variant" value={variant.sku ?? ""} checked={sku === variant.sku} onChange={() => setSku(variant.sku ?? "")} /><span>{[variant.color, variant.size].filter(Boolean).join(" · ") || variant.sku}</span></label>)}</div></fieldset> : null}
    <button className={styles.primary} disabled={!purchasable || inCart} onClick={() => { if (!selected.sku || selected.price === null) return; const key = selected.productGroupId ?? selected.id; addCartItem({ productId: selected.id, sku: selected.sku, name: selected.name, image: selected.images[0]?.url ?? null, price: selected.price, currency: selected.currency, color: selected.color, size: selected.size, quantity: 1, href: `/product/${encodeURIComponent(key)}`, maxQuantity: selected.stock }); }} type="button">{inCart ? "У кошику" : purchasable ? "Додати в кошик" : "Недоступно для замовлення"}</button>
    {inCart ? <p className={styles.notice} role="status">Товар додано. <Link href="/cart"><u>Перейти до кошика</u></Link></p> : null}
