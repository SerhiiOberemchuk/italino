...
Тому каталог кешується (`"use cache: remote"`), а не запитується на кожен перегляд.
...
CRM має webhook-доставку (QStash). Схема: CRM → `POST {SITE}/api/revalidate`
з `x-revalidate-secret` → `revalidateTag("catalog")`. Endpoint на сайті вже
реалізований; у CRM треба налаштувати виклик і спільний `REVALIDATE_SECRET`.
Як запасний механізм індекс каталогу оновлюється у фоні кожні 5 хвилин,
а категорії та точкові дані моделі/SKU використовують профіль `minutes`.
...
не виконується, щоб випадково не опублікувати весь каталог workspace. Усі
сторінки CRM читаються пачками й одразу групуються за `productGroupId`; у
remote cache зберігається компактний індекс моделей. Каталог, пошук, фільтри
та пагінація працюють по всьому асортименту. Сторінка товару запитує лише
варіанти свого `productGroupId`, а кошик/checkout — конкретні SKU.
Категорії, бренди, навігація, сторінки категорій, sitemap і товарні зображення
головної будуються з відповідей CRM. Сайт залишає лише категорії та бренди,
використані видимими товарами складу `OBRIYM_WAREHOUSE_ID`; для вкладених категорій
також зберігаються потрібні батьківські вузли дерева.
